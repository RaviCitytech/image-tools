...
window.Tools.convert = {
    init(uploadArea, editorArea, appState, config = {}) {
        let originalFiles = [];
        let firstImage = null;
        ...
        function showEditor() {
            ...
            // If config has fixed target, use it
            let select;
            if (config.targetFormat) {
                const lbl = document.createElement('label');
                lbl.className = 'control-label';
                // Extract format name for display (e.g., "WEBP")
                const fmtName = config.targetFormat.split('/')[1].toUpperCase();
                lbl.innerText = `Converting to ${fmtName}`;

                // Create a dummy object to mimic select behavior
                select = { value: config.targetFormat };

                formatContainer.appendChild(lbl);
            } else {
                const lbl = document.createElement('label');
                lbl.className = 'control-label';
